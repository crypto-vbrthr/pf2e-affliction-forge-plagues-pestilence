import {
  PLAGUES_PESTILENCE_CONTENT_VERSION,
  PLAGUES_PESTILENCE_DEFINITIONS,
  PLAGUES_PESTILENCE_MODULE_ID
} from "./content.js";

const MODULE_ID = PLAGUES_PESTILENCE_MODULE_ID;
const CORE_MODULE_ID = "pf2e-affliction-forge";
const PROVIDER_ID = "plagues-pestilence";
const LIBRARY_ID = "plagues-pestilence.core";
const PACK_NAME = "affliction-forge-plagues-pestilence";
...
async function createManagedPack() {
  ...
  try {
    return await CompendiumCollection.createCompendium({
      name: PACK_NAME,
      label: localize("PF2E_AFFLICTION_PP.Module.Library", "Plagues & Pestilence"),
      type: "Item",
      package: "world",
      packageName: "world",
      system: "pf2e"
...
function addonFlags() {
  return {
    managed: true,
    contentVersion: PLAGUES_PESTILENCE_CONTENT_VERSION,
...
async function syncManagedPack(api, pack) {
  ...
  const creates = [];
  const updates = [];
  for (const definition of PLAGUES_PESTILENCE_DEFINITIONS) {
...
function registerProvider(api) {
  ...
  api.providers.register({
    id: PROVIDER_ID,
    label: localize("PF2E_AFFLICTION_PP.Module.Title", "PF2E Affliction Forge: Plagues & Pestilence"),
    moduleId: MODULE_ID,
    version: PLAGUES_PESTILENCE_CONTENT_VERSION,
    metadata: {
      themes: ["disease", "plague", "pestilence"],
    ...
    libraries: [{
      ...
      description: localize("PF2E_AFFLICTION_PP.Module.LibraryDescription", "Original diseases with semantic Creature Forge tags."),
      metadata: {
        themes: ["disease", "plague", "pestilence"],
        semanticTagContract: "1.0.0",
        creatureForgeReady: true,
...
async function initialize(api) {
  ...
  console.info(`${MODULE_ID} | Content synchronized`, {
    ...result,
    total: PLAGUES_PESTILENCE_DEFINITIONS.length,
    collection: PACK_COLLECTION,
    version: PLAGUES_PESTILENCE_CONTENT_VERSION
  });
}
...
Hooks.once("pf2eAfflictionForgeReady", (api) => {
  initialize(api).catch((error) => {
    ...
    globalThis.ui?.notifications?.error?.(
      localize("PF2E_AFFLICTION_PP.Module.SyncFailed", "Plagues & Pestilence could not synchronize its library. See console.")
    );
  });
});
