...
const ADDON = "pf2e-affliction-forge-plagues-pestilence";
const COLLECTION = "world.affliction-forge-plagues-pestilence";
...
test("startup provisions, synchronizes, and registers the provider library", async () => {
  ...
  await readyHandler(api);
  await new Promise((resolve) => setTimeout(resolve, 10));
  assert.equal(index.length, 28, "second sync updates in place and removes stale managed content");
