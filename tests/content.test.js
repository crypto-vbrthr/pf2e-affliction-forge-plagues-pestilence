...
import {
  ...
  createPlaguesPestilenceDefinitions
...
test("0.1.2 ships 28 original disease definitions", () => {
  assert.equal(PLAGUES_PESTILENCE_DEFINITIONS.length, 28);
  assert.ok(PLAGUES_PESTILENCE_DEFINITIONS.every((definition) => definition.afflictionType === "disease"));
});

test("every definition validates against Affliction Forge schema v2", () => {
  const effectValidator = () => ({ valid: true, issues: [] });
  for (const source of PLAGUES_PESTILENCE_DEFINITIONS) {
...
test("all diseases use the GM Core level-based DC baseline", () => {
  for (const definition of PLAGUES_PESTILENCE_DEFINITIONS) {
...
test("semantic tags are canonical, in-contract, and Creature Forge useful", () => {
  for (const definition of PLAGUES_PESTILENCE_DEFINITIONS) {
    ...
    assert.ok(definition.themes.includes("theme:disease"), `${definition.id} lacks theme:disease`);
...
test("all i18n content tokens resolve in German and English", () => {
  const tokens = [...new Set(PLAGUES_PESTILENCE_DEFINITIONS.flatMap((definition) => collectI18nTokens(definition)))];
...
test("definition factory returns independent clones", () => {
  const a = createPlaguesPestilenceDefinitions();
  const b = createPlaguesPestilenceDefinitions();
